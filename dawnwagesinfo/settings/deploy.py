...
#     }
# }
if os.getenv('DATABASE_URL') and not os.getenv('HOST'):
    # A connection URL with a username and password (e.g. a Heroku Postgres
    # add-on). Only used when HOST isn't set, so your own Aurora settings win.
    DATABASES = {
    ...
    # Your own Aurora cluster with IAM authentication: the password is a
    # short-lived token generated by the custom backend
    # (see dawnwagesinfo/custom_backends). Needs HOST, NAME and DB_USER.
    DATABASES = {
        'default': {
            'ENGINE': 'dawnwagesinfo.custom_backends',
            'NAME': os.getenv('NAME'),
